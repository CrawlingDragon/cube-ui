...
`CascadePicker` component is used to implement the cascading change between picker columns. So, what is the effect of cascade? For example, when using an province-city-area picker, you may want the city column be cascade to the change of province, at meanwhile, the area column be cascade to the change of city. The `CascadePicker` could help you do that.

### Example

- Basic usage

  Comparing to `Picker`, the structure of required props data is different for `CascadePicker`. The data structure of `Picker` is a two-dimensional array, and each column corresponds to an array.
